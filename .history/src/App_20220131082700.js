...
class App extends React.Component {
    ...
    getYoutubeAuthUrl() {
        this.setState({youtubeRedirect: true}, () => {
            localStorage.setItem("state", JSON.stringify(this.state))
            axios.get("http://localhost:3000//youtube/getAuthUrl").then(response => {
    ...
    getYoutubeAccessToken() {
        return new Promise((resolve, reject) => {
            try {
                ...
                axios.get("http://localhost:3000//youtube/token", {params: {
                    authCode:authCode,
                    authState:authState
                }}).then(response => {
    ...
    async findYoutubePlaylist(playlistInput) {
        ...
        if (tokenFound || hasCreds) {
            axios.get("http://localhost:3000//youtube/playlist", {params: {
    ...
    async bulkSearch(trackNames) {
        ...
        let searchPromises = []
        axios.get("http://localhost:3000//spotify/searchToken").then(response => {
            for (let i = 0; i < trackNames.length; i++) {
                searchPromises.push(
                    axios.get("http://localhost:3000//spotify/search", {params: {
                        query: trackNames[i],
                        token: response.data
                    }})
                )
    ...
    getSpotifyAuthUrl() {
        this.setState({spotifyRedirect: true}, () => {
            axios.get("http://localhost:3000//spotify/getAuthUrl").then(response => {
                localStorage.setItem("state", JSON.stringify(this.state))
                let authUrl = response.data
                window.location.replace(authUrl)
            })
    ...
    getSpotifyAccessToken(authCode) {
        return new Promise((resolve, reject) => {
            if (this.state.spotifyRedirect) {
                axios.get("http://localhost:3000//spotify/token", {params: {
                    authCode:authCode,
                }}).then(response => {
                    this.setState({
    ...
    async savePlaylist() {
        return new Promise(async (resolve, reject) => {
            ...
            console.log("THREE")
            axios.get("http://localhost:3000//spotify/savePlaylist", {params: {
                playlistName: this.state.playlist.playlistName,
