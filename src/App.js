...
class App extends React.Component {
      ...
      await new Promise((resolve, reject) => {
        if (trackPosition + 1 < tracks.length) {
          this.setState({ activeTrack: tracks[trackPosition + 1] }, () => {
            console.log(tracks[trackPosition + 1]);
            document
              .getElementById(tracks[trackPosition + 1].key)
              .setAttribute("style", "border: thin solid yellow");
            resolve();
          });
  ...
  handleAdd(track) {
    ...
    if (
      ...
      let searchIndex = this.state.searchResults
        .map((searchTrack) => {
          return searchTrack.key;
        })
        .indexOf(track.key);
      ...
      this.setState({ searchResults: newSearchResults }, () => {
        ...
        if (!this.state.activeTrack.hasOwnProperty("key")) {
          ...
          let activeIndex = this.state.playlist.playlistTracks
            .map((playlistTrack) => {
              return playlistTrack.key;
            })
            .indexOf(activeTrack.key);
          ...
          this.setState(
            ...
            () => {
              document
                .getElementById(activeTrack.key)
                .setAttribute(
                  "style",
                  "border: none border-bottom: 1px solid rgba(256, 256, 256, 0.8)"
                );
              document
                .getElementById(track.key)
                .setAttribute("style", "border: thin solid yellow");
  ...
  setActiveTrack(event) {
    ...
    this.setState(
      ...
      () => {
        document
          .getElementById(targetTrack.key)
          .setAttribute("style", "border: thin solid yellow");
        localStorage.setItem("state", JSON.stringify(this.state));
      }
    );
  ...
  navigateDown() {
    let tracks = this.state.playlist.playlistTracks;
    let currentIndex = tracks
      .map((track) => {
        return track.key;
      })
      .indexOf(this.state.activeTrack.key);
    ...
    if (currentIndex + 1 >= tracks.length) {
      ...
      document
        .getElementById(tracks[currentIndex].key)
        .setAttribute(
          "style",
          "border: none border-bottom: 1px solid rgba(256, 256, 256, 0.8)"
        );
      document
        .getElementById(tracks[currentIndex + 1].key)
        .setAttribute("style", "border: thin solid yellow");
      this.setState(
        {
  ...
  getYoutubeAuthUrl() {
    this.setState({ youtubeRedirect: true }, () => {
      localStorage.setItem("state", JSON.stringify(this.state));
      axios
        .get(`${process.env.REACT_APP_SERVER_URL}/youtube/getAuthUrl`)
        .then((response) => {
  ...
  getYoutubeAccessToken() {
    return new Promise((resolve, reject) => {
      try {
        ...
        let authState = decodeURIComponent(
          window.location.href.match(/state=([^&]*)/)[1]
        );
        axios
          .get(`${process.env.REACT_APP_SERVER_URL}/youtube/token`, {
            params: {
              authCode: authCode,
              authState: authState,
            },
  ...
  async findYoutubePlaylist(playlistInput, event, button) {
    ...
    if (tokenFound || hasCreds) {
      axios
        .get(`${process.env.REACT_APP_SERVER_URL}/youtube/playlist`, {
          params: {
  ...
  async bulkSearch(trackNames, event, button) {
    ...
    let searchPromises = [];
    axios
      .get(`${process.env.REACT_APP_SERVER_URL}/spotify/searchToken`)
      .then((response) => {
        for (let i = 0; i < trackNames.length; i++) {
          searchPromises.push(
            new Promise((resolve, reject) => {
              axios
                .get(`${process.env.REACT_APP_SERVER_URL}/spotify/search`, {
                  params: {
                    query: trackNames[i],
                    token: response.data,
                    // Fill progress bar
                ...
                .then((response) => {
                  button.setState(
                    {
                      totalSearchPromisesResolved:
                        button.state.totalSearchPromisesResolved + 1,
                    },
                    () => {
                      let currentProgress = Math.round(
                        (button.state.totalSearchPromisesResolved /
                          button.state.totalSearchPromises) *
                          100
                      );
        ...
        Promise.all(searchPromises).then((responses) => {
          ...
          for (let i = 0; i < responses.length; i++) {
            ...
            if (
              ...
              !recommendation.hasOwnProperty("key")
            ) {
              ytPlaylist.push({
                notFound: true,
  ...
  getSpotifyAuthUrl() {
    this.setState({ spotifyRedirect: true }, () => {
      axios
        .get(`${process.env.REACT_APP_SERVER_URL}/spotify/getAuthUrl`)
        .then((response) => {
          localStorage.setItem("state", JSON.stringify(this.state));
          let authUrl = response.data;
          window.location.replace(authUrl);
  ...
  getSpotifyAccessToken(authCode) {
    return new Promise((resolve, reject) => {
      if (this.state.spotifyRedirect) {
        axios
          .get(`${process.env.REACT_APP_SERVER_URL}/spotify/token`, {
            params: {
              authCode: authCode,
            },
          })
  ...
  async savePlaylist(event) {
    return new Promise(async (resolve, reject) => {
      ...
      let playlistTrackUris = this.state.playlist.playlistTracks.map(
        (track) => {
          if (track.notFound) {
            return undefined;
          } else {
      ...
      axios
        .get(`${process.env.REACT_APP_SERVER_URL}/spotify/savePlaylist`, {
          params: {
            playlistName: this.state.playlist.playlistName,
            playlistTracks: encodeURIComponent(
              JSON.stringify(currentTrackUris)
            ),
          ...
          if (response.status < 400) {
            ...
            while (true) {
              ...
              if (currentTrackUris.length === 0) {
                ...
                axios.get(
                  `${process.env.REACT_APP_SERVER_URL}/spotify/savePlaylist`,
                  {
                    params: {
                      playlistName: this.state.playlist.playlistName,
                      playlistTracks: encodeURIComponent(
